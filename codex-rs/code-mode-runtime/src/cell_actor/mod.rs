    ...
    loop {
        ...
        tokio::select! {
            ...
            maybe_command = async {
                ...
                if runtime_paused
                    && matches!(mode, ObserveMode::YieldAfter(_) | ObserveMode::UntilYield)
                {
                    pending_frontier_ready = false;
                ...
                match event {
                    ...
                    RuntimeEvent::YieldRequested => {
                        let yield_observer = matches!(
                            observer.as_ref().map(|observer| observer.mode),
                            Some(ObserveMode::YieldAfter(_) | ObserveMode::UntilYield)
                        );
                        if yield_observer {
                            yield_timer = None;
                            restore_undelivered_yield(
...
fn observer_timer(observer: &Observer) -> Option<std::pin::Pin<Box<tokio::time::Sleep>>> {
    match observer.mode {
        ...
        ObserveMode::UntilYield | ObserveMode::PendingFrontier => None,
    }
}
...
fn resume_for_observation(
    ...
    if *runtime_paused {
        let control = match mode {
            ObserveMode::YieldAfter(_) | ObserveMode::UntilYield => RuntimeControlCommand::Continue,
