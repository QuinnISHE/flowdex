...
impl SessionInner {
    ...
    pub(super) async fn wait(
        ...
        self.wait_inner(request, /*until_yield*/ false).await
    }
    ...
    async fn wait_inner(
        ...
        let mut client = self.client();
        let response = if until_yield {
            tokio::select! {
                biased;
                _ = self.stopped.cancelled() => {
                    Err("gRPC code-mode session closed".to_string())
                }
                response = client.wait(request) => {
                    response.map_err(|error| deadline::failure("wait", error))
                }
            }
        } else {
            deadline::request(self, "wait", runtime_timeout, client.wait(request)).await
        };
        cancellation.disarm();
