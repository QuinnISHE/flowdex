...
impl AgentControl {
    ...
    pub(crate) async fn submit_compaction(
        ...
        let submission_id = self
            .handle_thread_request_result(
                agent_id,
                &state,
                state.send_op(agent_id, op, None, None).await,
            )
            .await?;
    ...
    pub(crate) async fn submit_input_operation(
        ...
        let thread = state.get_thread(agent_id).await?;
        let result = match thread
            .start_turn_if_idle(TurnInputRequest::user_input(input).on_start(start_options))
            .await
        {
            Ok(StartIfIdleSubmission::Started { turn_id }) => Ok(turn_id),
            Ok(StartIfIdleSubmission::NotSubmitted { reason }) => Err(CodexErr::InvalidRequest(
                format!("turn input was not submitted: {reason:?}"),
            )),
            Err(err) => Err(err),
        };
