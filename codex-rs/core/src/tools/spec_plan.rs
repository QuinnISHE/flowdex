...
pub(crate) fn add_flowdex_tools(turn_context: &TurnContext, registry: &mut ToolRegistry) {
    let agent_tools_enabled = collab_tools_enabled(turn_context);
    ...
    let verification_enabled = registry.entries().any(|tool| {
        let name = tool.runtime.tool_name();
    ...
    let mut nested_specs = registry
        ...
        .collect::<Vec<_>>();
    nested_specs.push(FlowdexCheckRulesHandler.spec());
    nested_specs.push(FlowdexSignalHandler.spec());
    if agent_tools_enabled {
        nested_specs.extend([
    ...
    nested_specs.push(FlowdexRunWorkflowHandler::new(Vec::new()).spec());
    registry.add_with_exposure(
