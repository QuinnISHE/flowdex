...
use crate::tools::flowdex::FlowdexCreateTaskHandler;
use crate::tools::flowdex::FlowdexExecCommandHandler;
use crate::tools::flowdex::FlowdexQueueTaskHandler;
use crate::tools::flowdex::FlowdexResumeAgentHandler;
...
use crate::tools::flowdex::FlowdexSendMessageHandler;
use crate::tools::flowdex::FlowdexSignalHandler;
use crate::tools::flowdex::FlowdexSpawnAgentHandler;
use crate::tools::flowdex::FlowdexStartRunHandler;
use crate::tools::flowdex::FlowdexTaskIntegrateHandler;
use crate::tools::flowdex::FlowdexTaskRunAgentHandler;
use crate::tools::flowdex::FlowdexTaskVerifyHandler;
use crate::tools::flowdex::FlowdexVerifyHandler;
use crate::tools::flowdex::FlowdexWaitAgentHandler;
...
use crate::tools::flowdex::StartFlowdexWorkflowHandler;
...
pub(crate) fn build_tool_router(
    ...
    let mut hosted_specs = if crate::guardian::is_basic_session_source(&turn_context.session_source)
    {
        Vec::new()
    } else {
    ...
    apply_flowdex_agent_tool_exclusions(turn_context, &mut registry, &mut hosted_specs);
    add_flowdex_tools(turn_context, &mut registry);
...
pub(crate) fn apply_flowdex_agent_tool_exclusions(
    turn_context: &TurnContext,
    registry: &mut ToolRegistry,
    ...
    let removed = registry
        .entries()
        .filter_map(|tool| {
            let name = tool.runtime.tool_name();
            excluded
                .iter()
                .any(|excluded| excluded == &name.to_string())
                .then_some(name)
        })
        .collect::<Vec<_>>();
    for name in removed {
    ...
    hosted_specs.retain(|spec| !excluded.iter().any(|name| name == spec.name()));
}

pub(crate) fn add_flowdex_tools(turn_context: &TurnContext, registry: &mut ToolRegistry) {
    let agent_tools_enabled = collab_tools_enabled(turn_context, turn_context.model_info());
    registry.add_with_exposure(FlowdexCheckRulesHandler, ToolExposure::Hidden);
    registry.add_with_exposure(FlowdexSignalHandler, ToolExposure::Hidden);
    if agent_tools_enabled {
        registry.add_with_exposure(FlowdexSpawnAgentHandler, ToolExposure::Hidden);
        registry.add_with_exposure(FlowdexSendMessageHandler, ToolExposure::Hidden);
        registry.add_with_exposure(FlowdexWaitAgentHandler, ToolExposure::Hidden);
        registry.add_with_exposure(FlowdexResumeAgentHandler, ToolExposure::Hidden);
        registry.add_with_exposure(FlowdexCreateTaskHandler, ToolExposure::Hidden);
        registry.add_with_exposure(FlowdexTaskRunAgentHandler, ToolExposure::Hidden);
        registry.add_with_exposure(FlowdexTaskVerifyHandler, ToolExposure::Hidden);
        registry.add_with_exposure(FlowdexTaskIntegrateHandler, ToolExposure::Hidden);
        registry.add_with_exposure(FlowdexStartRunHandler, ToolExposure::Hidden);
        registry.add_with_exposure(FlowdexQueueTaskHandler, ToolExposure::Hidden);
        registry.add_with_exposure(FlowdexSealPhaseHandler, ToolExposure::Hidden);
        registry.add_with_exposure(FlowdexWaitRunHandler, ToolExposure::Hidden);
    }
    if review_report_tool_visible(turn_context.session_source.get_agent_path().as_ref()) {
        registry.add_with_exposure(FlowdexReviewReportHandler, ToolExposure::DirectModelOnly);
    }
    ...
    let verification_enabled = registry
        .entries()
        .any(|tool| tool.runtime.tool_name() == ToolName::plain("exec_command"));
    if verification_enabled {
        let exec_options = ExecCommandHandlerOptions {
            allow_login_shell: any_environment_allows_login_shell(&turn_context.environments),
        ...
        registry.add_with_exposure(
            FlowdexExecCommandHandler::new(exec_options),
            ToolExposure::Hidden,
        );
        registry.add_with_exposure(FlowdexVerifyHandler::new(), ToolExposure::Hidden);
    }

    let mut nested_specs = registry
        .entries()
        .filter(|tool| {
            tool.exposure != ToolExposure::DirectModelOnly && tool.exposure != ToolExposure::Hidden
        })
        .map(|tool| {
            (
                Arc::new(tool.runtime.spec()),
                Some(Arc::clone(&tool.runtime)),
            )
        })
        .collect::<Vec<_>>();
    nested_specs.push((Arc::new(FlowdexCheckRulesHandler.spec()), None));
    nested_specs.push((Arc::new(FlowdexSignalHandler.spec()), None));
    if agent_tools_enabled {
        nested_specs.extend([
            (Arc::new(FlowdexSpawnAgentHandler.spec()), None),
            (Arc::new(FlowdexSendMessageHandler.spec()), None),
            (Arc::new(FlowdexWaitAgentHandler.spec()), None),
            (Arc::new(FlowdexResumeAgentHandler.spec()), None),
            (Arc::new(FlowdexCreateTaskHandler.spec()), None),
            (Arc::new(FlowdexTaskRunAgentHandler.spec()), None),
            (Arc::new(FlowdexTaskVerifyHandler.spec()), None),
            (Arc::new(FlowdexTaskIntegrateHandler.spec()), None),
            (Arc::new(FlowdexStartRunHandler.spec()), None),
            (Arc::new(FlowdexQueueTaskHandler.spec()), None),
            (Arc::new(FlowdexSealPhaseHandler.spec()), None),
            (Arc::new(FlowdexWaitRunHandler.spec()), None),
        ]);
    }
    if verification_enabled {
        nested_specs.push((Arc::new(FlowdexVerifyHandler::new().spec()), None));
    }
    nested_specs.push((
        Arc::new(FlowdexRunWorkflowHandler::new(Vec::new()).spec()),
        None,
    ));
    registry.add_with_exposure(
        FlowdexRunWorkflowHandler::new(nested_specs.clone()),
        ToolExposure::Hidden,
    );
    registry.add_with_exposure(
        StartFlowdexWorkflowHandler::new(nested_specs),
        ToolExposure::DirectModelOnly,
    );
    registry.add_with_exposure(SaveFlowdexWorkflowHandler, ToolExposure::DirectModelOnly);
    registry.add_with_exposure(WaitFlowdexWorkflowHandler, ToolExposure::DirectModelOnly);
    registry.add_with_exposure(
        ContinueFlowdexWorkflowHandler,
        ToolExposure::DirectModelOnly,
    );
    registry.add_with_exposure(PublishFlowdexContextHandler, ToolExposure::DirectModelOnly);
    registry.add_with_exposure(ReadFlowdexContextHandler, ToolExposure::DirectModelOnly);
    registry.add_with_exposure(QueueFlowdexTaskHandler, ToolExposure::DirectModelOnly);
    registry.add_with_exposure(SealFlowdexPhaseHandler, ToolExposure::DirectModelOnly);
    registry.add_with_exposure(PauseFlowdexWorkflowHandler, ToolExposure::DirectModelOnly);
    registry.add_with_exposure(ResumeFlowdexWorkflowHandler, ToolExposure::DirectModelOnly);
}
