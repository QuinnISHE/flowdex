...
use crate::tools::spec_plan::add_flowdex_tools;
use crate::tools::spec_plan::append_source_tools;
use crate::tools::spec_plan::apply_flowdex_agent_tool_exclusions;
...
fn plan_with_model(
    ...
    let mut hosted_specs = append_source_tools(
        turn,
        model_info,
        &mut registry,
    ...
    apply_flowdex_agent_tool_exclusions(turn, &mut registry, &mut hosted_specs);
    add_flowdex_tools(turn, &mut registry);
...
fn set_feature(turn: &mut TurnContext, feature: Feature, enabled: bool) {
    ...
    // These tests exercise Codex's feature-derived backend selection, not the
    // independent Flowdex configuration override.
    config.flowdex_config.multi_agent_version = None;
    if enabled {
        config
            .features
            .enable(feature)
