...
- **Desktop app-backend installer (Batch 014, streamlined after completion):** the packaged `flowdex` binary installs or uninstalls itself with `flowdex install` and `flowdex uninstall`. It keeps the Codex app untouched, stores the copied backend under `$CODEX_HOME/flowdex/bin`, and selects it through the Windows user environment or a managed macOS login-profile block.
